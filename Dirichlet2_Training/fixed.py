...
def cost(p, x, y):
    ...
    l5 = model.f_vecty(p, 1.0, y) - jnp.sin(jnp.pi*y)
    return jnp.mean(l1**2) + jnp.mean(l2**2+l3**2+l4**2+l5**2)

xs = jnp.linspace(0.0,1.0,101)
ys = jnp.linspace(0.0,1.0,101)

model.train(xs, ys, cost, learning_rate=0.01, epochs=10001, momentum=0.99)
