...
class ODE_Solver:
    ...
    def train(self, xs, loss_function, epochs=1000, lr=0.1, beta=0.9):
        ...
        for epoch in range(epochs):
            ...
            s = [(beta*sw + (1-beta)*dw**2, beta*sb + (1-beta)*db**2) for (sw, sb), (dw, db) in zip(s, grads)]
            self.params = [(w - (lr*(sw**(-1/2)))*dw, b - (lr*(sb**(-1/2)))*db) for (w, b), (sw, sb), (dw, db) in zip(self.params, s, grads)]
...
if __name__ == '__main__':
    ...
    for i in range(50):
        ...
        t0 = time()
        losses = model.train(xs, loss, epochs=5000, lr=0.005, beta=0.9)
    ...
    url = '/Users/oliver/Desktop/DLDataGen/{}/'.format('rmsprop')
