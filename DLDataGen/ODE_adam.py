...
class ODE_Solver:
    ...
    def train(self, xs, loss_function, epochs=1000, lr=0.1, beta1=0.9, beta2=0.9):
        ...
        s = [(np.zeros((m, n)), np.zeros(n)) for m, n, in zip(self.layer_sizes[:-1], self.layer_sizes[1:])]
        losses = []
        ...
        for epoch in range(epochs):
            ...
            velocity = [(beta1*vw + (1-beta1)*dw, beta1*vb + (1-beta1)*db) for (vw, vb), (dw, db) in zip(velocity, grads)]
            s = [(beta2*sw + (1-beta2)*dw**2, beta2*sb + (1-beta2)*db**2) for (sw, sb), (dw, db) in zip(s, grads)]
            self.params = [(w - (lr*(sw**(-1/2)))*vw, b - (lr*(sb**(-1/2)))*vb) for (w, b), (sw, sb), (vw, vb) in zip(self.params, s, velocity)]
...
if __name__ == '__main__':
    ...
    for i in range(50):
        ...
        t0 = time()
        losses = model.train(xs, loss, epochs=5000, lr=0.01, beta1=0.9, beta2=0.9)
    ...
    url = '/Users/oliver/Desktop/DLDataGen/{}/'.format('adam')
