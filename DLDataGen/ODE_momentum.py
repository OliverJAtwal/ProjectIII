...
class ODE_Solver:
    ...
    def train(self, xs, loss_function, epochs=1000, lr=0.1, momentum=0.9):
        ...
        for epoch in range(epochs):
            ...
            velocity = [(momentum*vw + (1-momentum)*dw, momentum*vb + (1-momentum)*db) for (vw, vb), (dw, db) in zip(velocity, grads)]
            self.params = [(w - lr*vw, b - lr*vb) for (w, b), (vw, vb) in zip(self.params, velocity)]
...
if __name__ == '__main__':
    ...
    for i in range(50):
        ...
        t0 = time()
        losses = model.train(xs, loss, epochs=5000, lr=0.05, momentum=0.9)
    ...
    url = '/Users/oliver/Desktop/DLDataGen/{}/'.format('momentum')
