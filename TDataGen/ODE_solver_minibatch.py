...
class ODE_Solver:
    ...
    def train(self, xs, loss_function, epochs=1000, lr=0.1, mb=8):
        ...
        for epoch in range(epochs):
            ...
            for batch in self.minibatches(xs, mb):
                grads = grad_loss(self.params, batch)
                self.params = [(w - lr*vw, b - lr*vb) for (w, b), (vw, vb) in zip(self.params, grads)]

        return losses

    def minibatches(self, X, batchsize):
        np.random.shuffle(np.array(X))
        for start_idx in range(0, len(X), batchsize):
            end_idx = min(start_idx + batchsize, len(X))
...
if __name__ == '__main__':
    ...
    for i in range(50):
        ...
        losses = model.train(xs, loss, epochs=100001, lr=0.01, mb=4)
    ...
    y_data.to_csv('/Users/oliver/Desktop/DataGen/mbgd_y_data.csv')
    mse_data.to_csv('/Users/oliver/Desktop/DataGen/mbgd_mse_data.csv')
    loss_data.to_csv('/Users/oliver/Desktop/DataGen/mbgd_loss_data.csv')
