...
class ODE_Solver:
    ...
    def train(self, xs, loss_function, epochs=1000, lr=0.1):
        ...
        for epoch in range(epochs):
            ...
            for x in self.minibatches(xs):
                grads = grad_loss(self.params, x)
                self.params = [(w - lr*vw, b - lr*vb) for (w, b), (vw, vb) in zip(self.params, grads)]

        return losses

    def minibatches(self, X):
        for start_idx in range(0, len(X)):
            yield X[start_idx:start_idx+1]
...
if __name__ == '__main__':
    ...
    y_data.to_csv('/Users/oliver/Desktop/DataGen/sgd_y_data.csv')
    mse_data.to_csv('/Users/oliver/Desktop/DataGen/sgd_mse_data.csv')
    loss_data.to_csv('/Users/oliver/Desktop/DataGen/sgd_loss_data.csv')
